...
public class TilePlacerUI : MonoBehaviour {
    ...
    void Refresh() {
        ...
        var index = 0;
        foreach (Sprite sprite in TileSpriteManager.Instance.GetAllSprites(lastlayer)) {
    ...
    private void TilePlacerUI_OnClicked(object sender, EventArgs<UnityEngine.EventSystems.PointerEventData> e) {
        ...
        var name = ((UIButton)sender).GetComponent<SpriteRenderer>().sprite.name;
        TilePlacer.placer.SetType(name);
    }
