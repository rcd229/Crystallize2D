...
public class PhraseSetPostCompiler : EditorPostCompiler {
    ...
    public override void AfterCompile() {
        ...
        Debug.Log(string.Format("Project update! Unique learnable phrases: [{0}] Unique learnable words: [{1}]", phrases.Count, words.Count));
    }
    ...
    void UpdateKeySequences() {
        ...
        foreach (var ps in GameDataInitializer.phraseSets) {
