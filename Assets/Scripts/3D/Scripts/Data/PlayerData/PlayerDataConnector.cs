...
public class PlayerDataConnector {
    ...
    public static bool ContainsLearnedItem(PhraseSequenceElement word) {
        return PlayerData.Instance.WordStorage.ContainsFoundWord(word);
    }
    ...
    public static bool ContainsUncollectedItem(PhraseSequence phrase) {
        foreach (var w in phrase.PhraseElements) {
            if (CanLearn(new PhraseSequence(w), false)) {
                return true;
            }
        }
        return false;
    ...
    public static bool CanLearn(PhraseSequence phrase, bool message = true) {
        ...
        if (PlayerDataConnector.ContainsLearnedItem(phrase)) {
            if (message) {
                UILibrary.MessageBox.Get("You already have that word!");
            }
            return false;
        }
        ...
        if (!hasDictItem) {
            if (message) {
                UILibrary.MessageBox.Get("You can't learn that.");
            }
            return false;
        }
        ...
        if (phrase.IsWord) {
            if (CollectedWordCount >= PlayerData.Instance.Proficiency.Words) {
                if (message) {
                    UILibrary.MessageBox.Get("You can't learn any more words today. Review your words to earn more slots.");
                }
                return false;
            }
        } else {
            if (CollectedPhraseCount >= PlayerData.Instance.Proficiency.Phrases) {
                if (message) {
                    UILibrary.MessageBox.Get("You can't learn any more sentences today. Review your words to earn more slots.");
                }
                return false;
            }
        }
    ...
    public static int ConfidenceCost(PhraseSequence phrase) {
        ...
        foreach (var word in phrase.PhraseElements) {
            if (CanLearn(new PhraseSequence(word), false)) {
                amount++;
            }
        }
        return amount;
    ...
    public static void CollectItem(PhraseSequence item, bool addToCollected = true) {
        ...
        if (item.IsWord) {
            var word = item.Word;
            if (!word.IsDictionaryWord || PlayerData.Instance.WordStorage.ContainsFoundWord(word)) {
    ...
    public static IEnumerable<PhraseSequence> GetWordChoices(PhraseSequence ignoreWord) {
        ...
        var choices = PlayerData.Instance.WordStorage.FoundWords
    ...
    //TODO: make this depend on player data and serializable
