...
public class PhraseSetEditorWindow : EditorWindow {
    ...
    static void UpdateKeySequences() {
        ...
        foreach (var set in sets) {
            if (!PhrasePipeline.PhraseSets.ContainsKey(set.Name)) {
                continue;
            }

            var keys = PhrasePipeline.PhraseSets[set.Name];
    ...
    static void GetAllKeys() {
        PhraseSetCollectionGameData.LoadAll();
        foreach (var k in PhrasePipeline.PhraseSets.Keys) {
            foreach (var s in PhrasePipeline.PhraseSets[k]) {
